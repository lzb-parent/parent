...
import com.pro.common.module.service.message.util.NodeSmsUtils;
 ...
 */
@Slf4j
@Service("sysMsgChannelServiceSmsYunPian")
public class SysMsgChannelServiceSmsYunPian implements ISysMsgChannelService {
    ...
    private SysMsgRecordSendResult send(SysMsgChannelMerchant merchant, String title, String content, String phone) {
        try {
            JSONObject jo = NodeSmsUtils.sendSms(merchant.getBaseUrl(),
                    merchant.getApiKey(),
                    merchant.getApiPwd(),
                    phone,
                    content);
